...
object ParquetApp {
  def main(args: Array[String]): Unit = {
    var spark = SparkSession.builder().appName("SparkSessionApp")
      .master("lacal[2]").getOrCreate()

    /**
      * spark.read.format("parquet").load 这是标准写法
      */
    var userDF = spark.read.format("parquet").load("file:///home/hadoop/app/spark-2.1.0-bin/.parquet")

    userDF.printSchema()
    userDF.show()

    userDF.select("name","favorite_color").show()
    userDF.select("name","favorite_color").write.format("json").save("file:///home/hadoop")
